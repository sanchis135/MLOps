...
@click.option(
    "--month", default=1, prompt="Enter a month (1-12)", help="Month of the data"
)
...
def main(year, month, metrics, bucket, test):
    ...
    if metrics:
        ...
        sum_pred = np.sum(y_pred)  # <-- aquí sumas las predicciones
        logging.info(
            "Prediction metrics - Mean: %f, Std: %f, Min: %f, Max: %f, Sum: %f",
            mean_pred,
            std_pred,
            min_pred,
            max_pred,
